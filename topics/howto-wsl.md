...
## Prerequisites
This setup relies on having few directlories and scripts on the host machine (Windows):
...
Windows-home\wslconfig
```

In addition, it relies on the following software
- Windows terminal
...
- GIT for windows (so git bash can be the entry point for wsl commands)
- VScode 

## The Workflow for the first use or for a new distro
```
wsl -l -v                 ; to check wsl status
                          ; here we assume that wsl is not yet installed or 
                          ; we do not have the needed distribution image
wsl -l -o                 ; to check available distros; choose one
wsl --install -d <distro> ; start the installation 
```

The installation process will run in a separate window and will request setting up a new user:
```
...
From here, we need to complete the wsl configuration and to customize the image.
With the script:
```
...
### Installing and managing Linux distros
...
1. Preapare local reusable image of this distro
    ...
    ```
    wsl --export Ubuntu-20.04 ../.wsl-tars/2022-06-16-u20base
    ```
